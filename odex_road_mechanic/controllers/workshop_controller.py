...
class RoadMechanicWebsite(http.Controller, RoadMechanicMixin):
    ...
    # Workshop offers
    # ------------------------------------------------------------------
    @http.route(['/offers'], type='http', auth='public', website=True, sitemap=True)
        ...
        if post.get('service_id'):
            try:
                domain = expression.AND([
                    domain, [('service_id', '=', int(post['service_id']))]])
            except (TypeError, ValueError):
                pass
        step = self._orm_page_size()
        try:
    ...
    @http.route(['/offer/<string:offer_slug>'], type='http', auth='public',
                website=True, sitemap=False)
    def orm_offer_detail(self, offer_slug, **post):
        Offer = request.env['odex.road.mechanic.offer']
        offer = Offer.search(
            expression.AND([Offer._public_domain(), [('slug', '=', offer_slug)]]), limit=1)
        if not offer:
            raise NotFound()
        workshop = offer.workshop_id
        values = self._orm_common_values()
        values.update({
            'offer': offer,
            'workshop': workshop,
            'main_object': offer,
            'options': self._orm_search_options({}),
            'related_offers': Offer.search(
                expression.AND([
                    Offer._public_domain(workshop=workshop),
                    [('id', '!=', offer.id)]]), limit=3),
            'inquiry_sent': post.get('inquiry') == 'sent',
            'form_error': post.get('error'),
        })
        return request.render('odex_road_mechanic.offer_detail', values)
