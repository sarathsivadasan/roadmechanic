...
class RoadMechanicMessage(models.Model):
    ...
    workshop_id = fields.Many2one(
        ...
        help='The other side of the conversation. Empty until a provider is '
             'assigned on a marketplace request.')
    request_id = fields.Many2one(
        'odex.road.mechanic.request', string='Service Request',
        ondelete='cascade', index=True)
    offer_id = fields.Many2one(
        'odex.road.mechanic.part.offer', string='Offer', ondelete='cascade', index=True)
